...
use futures::Stream;
use grpcio::{ChannelBuilder, ChannelCredentialsBuilder, EnvBuilder, CallOption};
...
fn main() -> Result<(), Box<Error>> {
    ...
    let creds = ChannelCredentialsBuilder::new()
        .root_cert(fs::read("ca.pem")?)
        .cert(fs::read("cert.pem")?,
              fs::read("key.pem")?)
        .build();

    let channel = ChannelBuilder::new(env)
         // .override_ssl_target("your.tiller.cn.com")
        .secure_connect(TILLER_HOST, creds);
